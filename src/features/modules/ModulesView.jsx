...
import { ArrowRight, Rocket, Eye, ListTree, BrainCircuit, ScrollText, Handshake, ClipboardList, Settings } from 'lucide-react';
import { useLocation, useNavigate, useParams } from "react-router-dom";

const ModulesView = () => {
  const navigate = useNavigate();
  const location = useLocation();
  const { courseId, dayId } = useParams(); // Get parameters from the URL
  
  // Get the data passed through navigation state
...
    { title: "Sentence Pronunciation", content: "Manage and update Sentence Pronunciation.", path: "/sentence", icon: BrainCircuit, status: "Draft" },
    { title: "Practice Speaking", content: "Manage and update Practice Speaking.", path: "/practice", icon: ScrollText, status: "Active" },
    { title: "Conversation-Avatar to Student", content: "Manage and update Conversation-Avatar to Student.", path: "/avatartostudent", icon: Handshake, status: "Draft" },
    { title: "Conversation-Student To Avatar", content: "Manage and update Conversation-Student To Avatar.", path: "/studenttoavatar", icon: ClipboardList, status: "Active" },
    { title: "Quiz", content: "Manage and update Quiz.", path: "/quiz", icon: Settings, status: "Draft" },
  ];
  const [moduleStates, setModuleStates] = useState(initialModules);

  const handleModuleToggle = (moduleTitle) => {
    setModuleStates((prev) =>
      prev.map((mod) =>
        mod.title === moduleTitle
          ? { ...mod, status: mod.status === "Active" ? "Draft" : "Active" }
          : mod
      )
    );
  };
...
  const handleGoBack = () => {
    // Navigate back to the previous day's view
...
          <span className="text-indigo-600">{selectedDay || 'Loading...'}</span>
        </h2>
      </div>
...
      {moduleStates.map((module, index) => {
  ...
  return (
    <div
      ...
      onClick={() => navigate(module.path, { state: { selectedCourse, selectedDay } })} // ✅ navigate on card click
    >
      <div className="flex items-start justify-between">
        ...
        {/* ✅ Toggle still works without triggering card navigation */}
        <label
